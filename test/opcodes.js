...
describe('Opcodes', () => {
  ...
  it('should disassemble nop', () => {
    ...
    expect(disasm.disassemble()['main.asm']).to.equal(PREAMBLE + '    nop');
  });
  ...
  it('should disassemble increment registers', () => {
    ...
    Object.keys(incOpcodes).forEach((opcode) => {
      ...
      expect(disasm.disassemble()['main.asm']).to.equal(
  ...
  it('should treat unsupported opcodes as data', () => {
    ...
    unsupportedOpcodes.forEach((opcode) => {
      disasm.setUint8Array(new Uint8Array([].concat(opcode)));
      expect(disasm.disassemble()['main.asm']).to
  ...
  it('should disassemble sub', () => {
    ...
    expect(disasm.disassemble()['main.asm']).to.equal(`${PREAMBLE}    sub #0xff`);
  });
  ...
  it('should disassemble sbc', () => {
    ...
    expect(disasm.disassemble()['main.asm']).to.equal(`${PREAMBLE}    sbc a,#0xff`);
  });
  ...
  it('should disassemble cp', () => {
    ...
    expect(disasm.disassemble()['main.asm']).to.equal(`${PREAMBLE}    cp #0xff`);
  });
  ...
  it('should disassemble absolute jumps', () => {
    ...
    expect(disasm.disassemble()['main.asm']).to.equal(PREAMBLE + '    jp (hl)');
  ...
  it('should disassemble ex', () => {
    ...
    expect(disasm.disassemble()['main.asm']).to.equal(PREAMBLE + '    ex (sp),hl');

    disasm.setUint8Array(new Uint8Array([0xeb,]));
    expect(disasm.disassemble()['main.asm']).to.equal(PREAMBLE + '    ex de,hl');
  });
